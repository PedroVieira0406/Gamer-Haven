...
import br.unitins.topicos1.validation.SenhaIncompativelException;
...
@Override
    ...
    public void updatePassword(Long id, UpdatePasswordDTO dto) {
        ...
        if (cliente != null) {
            if (cliente.getLogin().getSenha().equals(hashSenhaAntiga)) {
                ...
                throw new SenhaIncompativelException("Senha antiga nao corresponde");
            }
        } else {
    ...
    public void updateUsername(Long id, UpdateUsernameDTO dto) {
        ...
        Cliente cliente = clienteRepository.findById(id);
        if (cliente != null) {
            String hashSenhaAntiga = hashService.getHashSenha(dto.password());
            if(hashSenhaAntiga == cliente.getLogin().getSenha()){
            cliente.getLogin().setName(dto.newUsername());;

        }else {
            throw new SenhaIncompativelException("Senha incorreta");
        }
        } 
        else {
            throw new NotFoundException();
